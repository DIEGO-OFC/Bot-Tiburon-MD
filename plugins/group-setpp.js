...
let handler = async (m, { conn, args }) => {
  ...
  if (/image/.test(mime)) {
    ...
    await conn.updateProfilePicture(m.chat, { url }).then(_ => m.reply('Exitosa actualización de imagen de perfil'))
  } else if (args[0] && args[0].match(URL_REGEX)) {
    await conn.updateProfilePicture(m.chat, { url: args[0] }).then(_ => m.reply('Exitosa actualización de imagen de perfi'))
  } else throw '¿Dónde están los medios?'
}
